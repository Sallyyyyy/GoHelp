--SQLQuery1 請列出每個借閱人每年借書數量，並依借閱人編號和年度做排序
	...
	SELECT BLR.KEEPER_ID AS [KeepId],MM.USER_CNAME AS [CName],MM.USER_ENAME AS [EName]
		   ,DATEPART(YEAR,BLR.LEND_DATE) AS [BorrowYear],COUNT(*) AS BorrowCnt
...
--SQLQuery7  請使用 PIVOT 語法列出 2016, 2017, 2018, 2019 各書籍類別的借閱數量比較 (動態分析所有年度)
	...
	SELECT @ColumnGroup = COALESCE(@ColumnGroup + ',' ,'' ) + QUOTENAME(CONVERT(NVARCHAR, Y1.year)) 
	FROM (
	...
	SELECT @ColumnSelect += ',ISNULL(['+CONVERT(NVARCHAR, Y2.year) + '],0) as CNT' + CONVERT(NVARCHAR, Y2.year)
	FROM (
